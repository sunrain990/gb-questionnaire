...
angular.module('staticVal',[])
    //.constant('URL','http://www.geminno.cn/project/index.php/api/')
    //.constant('SocketAddr','ws://121.41.123.2:5000')
    //.constant('NODE','http://121.41.123.2:5000');
    .constant('URL','http://test.geminno.cn/project/index.php/api/')
    .constant('SocketAddr','ws://115.28.91.11:5000')
    .constant('NODE','http://115.28.91.11:5000');
...
angular.module('components',['ui.sun.tpls','ui.sun.tabs','ui.sun.chatroom','ui.sun.faqs','ui.sun.faqs.form','ui.sun.sce','ui.sun.luntan','ui.sun.upload','ui.sun.askexpert']);
angular.module('ui.sun.tpls',[]);
//angular.module('ui.sun.tpls',['template/tabs/tab.html','template/tabs/tabset.html']);
...
//        replace:true,
            ...
            link:function(scope,element,attrs){
                ...
                //$rootScope.chatToast(
                //    '号外号外！'+
                //    '<span><img style="height: 25px;width: 25px;border-radius: 50%" src="http://www.geminno.cn/mis/include/imgs/thumtemp.jpg"></span><a href="www.baidu.com">6666</a>提问的问题已经得到<span><img src="http://www.geminno.cn/mis/include/imgs/thumtemp.jpg"></span><a href="www.baidu.com">6666</a>的回复！'
                //
                //    ,'info',20);
                ...
                function delayChat(){
                    ...
                    scope.chose = function(search){
                        ...
                        if(search.searchType == "全部"){
                            ...
                            scope.searches.search[0].active = true;
                            scope.searches.search[1].active = false;
                            scope.searches.search[2].active = false;
                            scope.searchType = "";
                        }else if(search.searchType == "私聊"){
                            var filter = {
                                type:"2"
                            };
                            scope.searches.search[0].active = false;
                            scope.searches.search[1].active = true;
                            scope.searches.search[2].active = false;
                            ...
                            scope.searches.search[0].active = false;
                            scope.searches.search[1].active = false;
                            scope.searches.search[2].active = true;
                            scope.searchType = filter;
                        }
                    };
                    ...
                    function onSocket() {
                        ...
                        //监听php系统
                        socket.on('c4p',function(data){
                            console.log(data,'c4p');
                            $rootScope.chatToast(data.text);
                        });
...
//                            faq2.img = reply.pics;
                ...
                scope.subqst = function(){
                    ...
                    $http.post(global.URL+'project/updatepost', pstmsg).success(function(data, status, headers, config) {
                        if(data.code == -1){
                            $rootScope.showToast(data.text,"post");
                            return;
                        }
                        ...
                        $http.post(global.URL+'project/updatefaq',faqmsg).success(function(data,status,headers,config){
                            if(data.code == -1){
                                $rootScope.showToast(data.text,"faq");
                                return;
                            }
                            faqid = data.data.id;
...
//                            projectid:63
                            ...
                            $http.post(global.URL+'user/addfaqtoblock',blkmsg).success(function(data,status,headers,config){
                                if(data.code == -1){
                                    if(!$rootScope.projectId){
                                        $rootScope.showToast(data.text,"该课程没有模块,不能添加问题！");
                                    }else{
                                        $rootScope.showToast(data.text,"后台错误！");
                                    }
                                    return;
                                }
                                var pstobj = {
                    ...
                    if(text == '全部'){
                        ...
                        var cb = function(){
                            ...
                            for(var i=0;i<scope.faqs1.length;i++){
                                ...
                                //    }
                                //}
                                console.log(scope.faqs1[i].replys);
                            }
...
angular.module('ui.sun.askexpert',[])
    .directive('askExpert',function($rootScope,$http){
        return {
            restrict:'EA',
            transclude:true,
            templateUrl:$rootScope.rooturl+"_pages/questionbank/listcourse/view/listcourse.html",
            link:function(scope,element,attrs){
                //function listcourse(){
                //    var msg = {
                //        page:1,
                //        pcount:16
                //    };
                //    $http.post(gem.apiprefix + "user/getpaidprojects",msg)
                //        .success(function(data){
                //            if(data.data.schedules){
                //                scope.myqidong = data.data.schedules;
                //            }else{
                ...
                //}
                //listcourse();
                var url = gem.apiprefix + "user/getpaidprojects",
                    data = {
                        page:1,
                        pcount:16,
                    };
                $http.post(url,data)
                    .success(function(data,status,headers,config){
                        if (data.code == -1) {
                            console.log(data,"无数据啊啊啊啊");
                            scope.pjData = data.data.schedules;
                        };
                        if(data.code == 1){
                            console.log(data,"getpaidprojects");
                            scope.pjData = data.data.schedules;
                            //背景图片
                            scope.getbg = function(a){
                                return  gem.getthemeimg(a);
                            }
                            //statenum为完成数
                            var getState = function(){
                                for (var i = 0; i < scope.pjData.length; i++) {
                                    scope.pjData[i].statenum = 0;
                                    var pjsblock = scope.pjData[i].sblocks;
                                    scope.pjsblocklength = pjsblock.length;
                                    pjsblock.forEach(function(o,index,obj){
                                        o.statenum = 0;
                                        for (var j = 0; j < obj.length; j++) {
                                            if(obj[j].state === "5" ){
                                                o.statenum += 1;
                                            }
                                            scope.pjData[i].statenum = o.statenum;
                                        }
                                    })
                                }
                            };
                            getState();
                            scope.setpercent = function (num1, num2) {
                                if(num2 == 0){
                                    return  0+"%";
                                }else{
                                    return parseInt(Math.round(num1/num2 * 10000) / 100.00 )+ "%";
                                }
                            }
                            console.log(scope.setpercent(0,1));

                            //分类
                            scope.getTecdir = function(name){
                                return gem.tecdires["i"+name];
                            }
                            //分页
...
//            tech.active = true;
//            $scope.currTechnology.active = false;
...
angular.module('ui.sun.upload',[])
    .controller('upCtrl',function($rootScope,$scope,$http,$window,global,Moment){

        $scope.fileresfn = function(jsn,tarjo) {
            if (!gem.urlVals.scheduleid) {
                $rootScope.showToast('scheduleid 不能为空！');
                return;
            }

            var url = gem.apiprefix + 'schedule/update';
            var msg = {
                id: gem.urlVals.scheduleid,
                file: jsn.data.id
            }
        ...
        $scope.docresfn = function(jsn,tarjo){
            if (!gem.urlVals.scheduleid) {
                $rootScope.showToast('scheduleid 不能为空！');
                return;
            }
        ...
        function initmyfile(){
            ...
            $http.post(gem.apiprefix+'schedule/get',msg)
                .success(function (data, status, headers, config) {
                    console.log(data);
                    if(data.data.file&&data.data.file.file){
                        $rootScope.uploaded['schedulefileup'] = [];
                        $rootScope.uploaded['schedulefileup'].push(data.data.file);
                    }
                    if(data.data.doc&&data.data.doc.file){
                        $rootScope.uploaded['scheduledocup'] = [];
                        $rootScope.uploaded['scheduledocup'].push(data.data.doc);
                    }
                }).error(function (data, status, headers, config) {
